...
For IEEE Transactions papers, the introduction should not promise evidence that the paper does not later deliver.

Apply these rules:

1. If the opening paragraph mentions scale, deployment, subject count, device count, benchmark count, or cost reduction, those numbers must already be backed by evidence.
2. If the evidence is not yet finalized, keep the introduction at the level of named settings, realistic scope, or explicit `needs evidence`.
3. Contribution bullets must map to later sections with actual artifacts such as benchmark tables, ablation tables, deployment results, or study-design summaries.
4. Avoid fake maturity signals such as `widely deployed`, `large-scale study`, or `substantial practical effectiveness` unless the scope is auditable.

Preferred patterns:

- `This problem is important in applications such as ...`
- `Existing methods remain limited by ...`
- `To address this issue, we propose ...`
- `We later evaluate this design on ...`

Do not front-load exact gains or real-world scale claims in the introduction unless they are already evidence-backed.
...
**Rule:** Skip the history lesson. State the specific technical problem directly.

### I2: The "Despite Significant Progress" Template
**Bad:** "Despite significant progress in deep learning, image segmentation remains a challenging task."
**Good:** "Prior subquadratic models fall short of Transformers on information-dense data." (Mamba)
**Rule:** State the specific bottleneck, not a generic "challenging task."

### I3: The "In Recent Years" Filler
**Bad:** "In recent years, attention mechanisms have attracted increasing attention."
**Good:** "Attention mechanisms enable models to focus on relevant parts of the input."
**Rule:** Delete "In recent years." State what the mechanism does.

### I4: The "Novel" Overuse
...
**Rule:** "Novel" once per introduction max. Use "proposed" or "our."

### I5: The "Comprehensive" Inflation
...
### I6: Template Contribution Bullets
...
### I8: Forced Motivation Sentences
**Bad:** "As an important research direction, image segmentation has attracted much attention."
**Good:** "Image segmentation enables autonomous driving, medical diagnosis, and robotics."
**Rule:** State why the task matters with concrete applications.

### I9: Vague Gap Identification
**Bad:** "Existing methods have some limitations."
**Good:** "Existing methods require O(n²) memory for sequence length n, limiting application to long sequences."
**Rule:** State the specific technical limitation with its consequence.

### I10: Missing Technical Insight
**Bad:** "We propose a method to solve this problem."
**Good:** "We observe that self-attention matrices are often sparse. We exploit this sparsity to reduce computation."
**Rule:** State the insight that enables your solution.

### Introduction De-AI Checklist

- [ ] No "With the rapid development of..." opening
- [ ] No "Despite significant progress" template
- [ ] No "In recent years" filler
- [ ] "Novel" appears at most once
- [ ] No "Comprehensive" without evidence
- [ ] Contribution bullets are specific and verifiable
- [ ] "However" appears at most once
- [ ] No forced motivation sentences
- [ ] Gap identification is specific with technical consequence
- [ ] Technical insight is explicitly stated
