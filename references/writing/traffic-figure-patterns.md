...
## 二十二、分类驱动绘图方法论（来自CCF-Figure）

### 22.1 论文类型 × 图示结构对照表

| 论文类型 | 推荐图示 | 交通预测示例 |
|---------|---------|-------------|
| A - 方法类 | 方法总览图 / 模型架构图 | STAEformer架构图、PDFormer模块图 |
| B - 机制/分析类 | 机制放大图 / 对比式方法图 | 传播延迟机制图、动态图学习过程 |
| C - 评测/基准类 | 评测流程图 / 数据构建流程图 | LargeST评测框架、LibCity流程 |
| D - Scaling Law | 趋势面板 / 实验矩阵 | 参数量vs性能、数据规模vs精度 |
| E - 机器人/具身智能 | 闭环反馈图 | 自动驾驶决策闭环 |
| F - 交叉方向 (AI for X) | 跨学科应用框架图 | 交通+气象、交通+事件 |
| G - 综述/位置论文 | 分类树 / 时间线 / 对比矩阵 | 方法演进时间线、技术对比矩阵 |

### 22.2 十一种图示类型

**类型1：方法总览图 (Method Overview)**
- 多阶段水平/垂直流程
- 核心创新模块用强调色
- 交通预测示例：Input → Spatial Encoding → Temporal Encoding → Fusion → Prediction

**类型2：系统框架图 (System Framework)**
- 分层/层级块状图
- 展示组件关系
- 交通预测示例：数据层 → 特征层 → 模型层 → 预测层

**类型3：模型架构图 (Model Architecture)**
- 逐层垂直或模块化框图
- 交通预测示例：GNN层 → Attention层 → FFN层 → 输出层

**类型4：机制示意图 (Mechanism Diagram)**
- 对单一子机制进行"放大镜"式局部展示
- 交通预测示例：传播延迟机制、动态图构建过程

**类型5：闭环反馈图 (Closed-Loop Feedback)**
- 圆形/椭圆形流程
- 感知→理解→规划→行动→观察→循环
- 交通预测示例：交通信号控制闭环

**类型6：多面板组合图 (Multi-Panel Composite)**
- 2x2、3x1 或非对称面板网格
- 交通预测示例：不同数据集的预测曲线对比

**类型7：实验规律总结图 (Experimental Trend Summary)**
- 趋势线面板、散点图、标注曲线
- 交通预测示例：参数量vs MAE散点图

**类型8：评测流程图 (Evaluation Pipeline)**
- 从原始数据到最终指标的顺序流程
- 交通预测示例：数据预处理 → 模型推理 → 指标计算 → 结果对比

**类型9：对比式方法图 (Contrastive Method Diagram)**
- 左（先前方法）vs 右（提出方法）
- 交通预测示例：静态图 vs 动态图、点预测 vs 概率预测

**类型10：数据构建流程图 (Data Construction Pipeline)**
- 来源→采集→过滤→标注→质控→最终数据集
- 交通预测示例：传感器数据采集→清洗→插补→标准化

**类型11：跨学科应用框架图 (Cross-Disciplinary Application Framework)**
- 领域数据区 + AI模型区 + 任务区 + 验证区
- 交通预测示例：交通数据 + 气象数据 + 事件数据 → 融合模型

---

## 二十三、五大翻车模式（来自CCF-Figure）

### 翻车1：只用摘要输入
- **问题**：摘要描述结果而非结构，导致随机方块
- **正确做法**：提供方法描述、模块关系、数据流方向

### 翻车2：模糊风格关键词
- **问题**："高端""科技感"等词会触发3D渲染和霓虹效果
- **正确做法**：使用具体的学术规范描述（"二维扁平化矢量"）

### 翻车3：未声明论文类型
- **问题**：方法/评测/系统论文有完全不同的视觉逻辑
- **正确做法**：先分类论文类型，再选择图示结构

### 翻车4：信息过载
- **问题**：顶级会议图表信息密集但有清晰主线，不是堆砌
- **正确做法**：单栏5-7模块/6-10箭头；全页8-12模块/12-18箭头

### 翻车5：装饰过度
- **问题**：学术质量来自秩序和克制，而非渐变和阴影
- **正确做法**：二维扁平化矢量，禁止3D透视、霓虹渐变、装饰纹理

---

## 二十四、视觉规范速查（来自CCF-Figure）

### 背景
- 纯白 `#FFFFFF` 或极浅灰 `#F5F5F5`
...
### 容量指南
- 单栏图：5-7个模块，6-10个箭头
- 全页图：8-12个模块，12-18个箭头

---

## 学术图表分类体系（19类）

**灵感来源：awesome-ai-research-writing 的图表推荐系统**

### 一、性能对比类

| 图表类型 | 适用场景 | 示例 |
|---------|---------|------|
| 1. 柱状图 (Bar Chart) | 多方法性能对比 | 不同方法在某数据集上的精度对比 |
| 2. 分组柱状图 (Grouped Bar) | 多维度对比 | 不同方法在多个数据集上的对比 |
| 3. 折线图 (Line Chart) | 趋势对比 | 不同方法在不同设置下的性能趋势 |
| 4. 雷达图 (Radar Chart) | 多指标综合对比 | 不同方法在多个评估维度上的表现 |

### 二、趋势/收敛类

| 图表类型 | 适用场景 | 示例 |
|---------|---------|------|
| 5. 学习曲线 (Learning Curve) | 训练过程可视化 | 损失函数随epoch的变化 |
| 6. 收敛曲线 (Convergence Curve) | 算法收敛性 | 不同优化器的收敛速度对比 |
| 7. 缩放曲线 (Scaling Curve) | 模型缩放定律 | 性能随模型大小的变化 |

### 三、模型评估类

| 图表类型 | 适用场景 | 示例 |
|---------|---------|------|
| 8. 消融实验柱状图 (Ablation Bar) | 组件贡献分析 | 移除各组件后的性能变化 |
| 9. 参数敏感性图 (Sensitivity Plot) | 超参数影响 | 学习率/批大小对性能的影响 |
| 10. 效率-精度散点图 (Efficiency-Accuracy Scatter) | 权衡分析 | 不同方法的精度vs推理速度 |

### 四、数据关系类

| 图表类型 | 适用场景 | 示例 |
|---------|---------|------|
| 11. 散点图 (Scatter Plot) | 两变量关系 | 预测值vs真实值 |
| 12. 热力图 (Heatmap) | 矩阵数据可视化 | 注意力权重矩阵、混淆矩阵 |
| 13. 相关性矩阵 (Correlation Matrix) | 特征关系 | 不同特征之间的相关性 |

### 五、统计分布类

| 图表类型 | 适用场景 | 示例 |
|---------|---------|------|
| 14. 箱线图 (Box Plot) | 分布对比 | 不同方法的性能分布 |
| 15. 小提琴图 (Violin Plot) | 分布形状 | 预测误差的分布形状 |
| 16. 直方图 (Histogram) | 频率分布 | 预测误差的频率分布 |

### 六、复合布局类

| 图表类型 | 适用场景 | 示例 |
|---------|---------|------|
| 17. 多面板图 (Multi-panel) | 多角度展示 | 同一方法在不同数据集上的结果 |
| 18. 子图网格 (Subplot Grid) | 系统对比 | 多个方法的定性结果对比 |
| 19. 混合布局 (Mixed Layout) | 复杂展示 | 定量结果+定性可视化的组合 |
