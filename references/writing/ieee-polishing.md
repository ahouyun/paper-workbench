...
### 规则 47: Introduction的逻辑结构（六段式漏斗）

顶级论文的Introduction遵循以下逻辑递进：
...
验证：极简MLP架构在正确建模时空特征时，性能媲美复杂GNN
```

---

### 规则 48: Method与Introduction的对应关系

**原则：** Introduction中提出的每个问题，Method中必须有对应的解决方案。
...
**反面示例（常见问题）：**
- ❌ Introduction提了三个问题，Method只解决了两个
- ❌ Introduction说"现有方法忽略了X"，但Method中也没有显式建模X
- ❌ 消融实验的变体与Introduction的问题不对应

---

### 规则 49: Experiment部分的论证逻辑
...
| 陷阱 | 表现 | 解决方案 |
|------|------|---------|
| **选择性报告** | 只报告最好的结果 | 报告所有数据集的结果 |
| **不公平对比** | 基线设置不一致 | 使用相同的实验设置 |
| **过度声称** | "我们的方法是最好的" | "在XX数据集上，我们的方法..." |
| **忽略局限** | 不讨论方法的局限性 | 明确说明适用范围 |
| **数据泄露** | 测试集信息泄露到训练集 | 严格划分数据集 |

---

### 规则 51: 逻辑严谨性检查清单

- [ ] Introduction的每个问题在Method中都有对应解决方案
- [ ] 消融实验的变体与Introduction的问题对应
- [ ] 所有数据集的结果都已报告（不选择性报告）
- [ ] 基线方法的实验设置一致
- [ ] 没有过度声称（使用hedging语言）
- [ ] 讨论了方法的局限性
...
> 更新时间：2026-06-20
> 来源：academic-research-skills (Imbad0202), nature-skills (Yuan1z0825), traffic-logic-rigor
