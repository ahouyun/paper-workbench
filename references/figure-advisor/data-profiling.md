...
数据剖析是"思考-绘制"工作流的第 1 步。本文档讲：**怎么读数据剖析报告 + 怎么把每一条事实翻译成画图决策**。
...
- 实验编号是数字（如 1-10），被认成 ordinal，**但你想当分类**。手动转成字符串
...
1. **看列类型**：识别对不对？错了手动改
2. **看样本量**：每组 n 是多少？小样本警告？
3. **看分布形态**：偏度多少？需要对数轴吗？
4. **看分组结构**：单分组还是多分组？平衡吗？
5. **看相关性**：哪些变量高度相关？决定 pairplot 子集
6. **看缺失值**：缺失率多少？需要在图注交代吗？
7. **查图表选择决策框架**：根据数据形态+论证目标选图型
8. **查避坑清单**：避免常见错误
