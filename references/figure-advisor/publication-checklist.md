...
每张图投稿前**逐条勾选**。任何 FAIL 必须重新出图。

> 这份清单只查**形式合规**（尺寸、DPI、字号、误差是否写明）。**语义层面**的避坑（均值柱掩盖分布、双 Y 轴误导、饼图等）在 `viz-pitfalls.md` — 提交前两份清单都要过。
...
- [ ] 不出现负号方框（`axes.unicode_minus = False`）
...
## 语义合规（viz-pitfalls 交叉检查）

形式合规 ≠ 语义合规。下面 8 条是 `viz-pitfalls.md` 里 15 条避坑清单的**精选 must-pass 项**：
